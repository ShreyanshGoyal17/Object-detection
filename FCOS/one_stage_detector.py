...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            stem_cls.append(nn.Conv2d(c[i], c[i+1], 3, padding='same'))
            stem_box.append(nn.Conv2d(c[i], c[i+1], 3, padding='same'))
            nn.init.normal_(stem_cls[2*i].weight, 0, 0.01)
            nn.init.constant_(stem_cls[2*i].bias, 0)
            nn.init.normal_(stem_box[2*i].weight, 0, 0.01)
            nn.init.constant_(stem_box[2*i].bias, 0)
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(c[-1], num_classes, 3, padding='same')
        self.pred_box = nn.Conv2d(c[-1], 4, 3, padding='same')
        self.pred_ctr = nn.Conv2d(c[-1], 1, 3, padding='same')
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, feat in feats_per_fpn_level.items():
            class_logits[key] = self.pred_cls(self.stem_cls(feat)).flatten(2).transpose(1, 2)
            boxreg_deltas[key] = self.pred_box(self.stem_box(feat)).flatten(2).transpose(1, 2)
            centerness_logits[key] = self.pred_ctr(self.stem_box(feat)).flatten(2).transpose(1, 2)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    deltas = torch.zeros_like(gt_boxes[:, :4]) # shape (N, 4) (LTRB)
    ...
    bg = gt_boxes[:, 0] == -1 # background idx
    deltas[bg] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    bg = deltas[:, 0] == -1 # background idx
    LTRB = deltas.clamp(min=0) * stride
    output_boxes = torch.zeros_like(LTRB) # (x1, y1, x2, y2)
    output_boxes[:, 0] = locations[:, 0] - LTRB[:, 0] # x1 = xc - L
    output_boxes[:, 1] = locations[:, 1] - LTRB[:, 1] # y1 = yc - T
    output_boxes[:, 2] = locations[:, 0] + LTRB[:, 2] # x2 = xc + R
    output_boxes[:, 3] = locations[:, 1] + LTRB[:, 3] # y2 = yc + B
    
    # Output box should be the location itself for background (delta == -1)
    output_boxes[bg, 0] = output_boxes[bg, 2] = locations[bg, 0]
    output_boxes[bg, 1] = output_boxes[bg, 3] = locations[bg, 1]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for i in range(B):
            ...
            for key in matched_gt_boxes[i]:
                matched_gt_deltas[i][key] = fcos_get_deltas_from_locations(locations_per_fpn_level[key], matched_gt_boxes[i][key], stride=self.backbone.fpn_strides[key])
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # shape (batchsize * sum_resolution,)
        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas.reshape(-1, 4)).view(-1)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_locations = level_locations[conf_idx]
